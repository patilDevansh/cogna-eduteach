...
- Dual-control skipped "for speed" ✓ (service + HTTP endpoints implemented; prod auth deferred)
- Policy golden suites mutating global PROMOTED row in parallel ✓ (policy-suite-lock serializes A02–A10)
- Modality tests using non-canonical unit ID `linear-equations` ✓ (fixed to `linear-equations-one-variable`)

## Enforced in P0/P1 Documentation Pass (2026-07-13) + Implementation (2026-07-14)

- [x] Policy governance: dual-control promote workflow defined (Policy Engineer + Policy Approver; emergency rollback single-actor) — **Service + HTTP endpoints implemented**
...
- **Schema:** ✓ Complete (subjects, modality_assets, modality_outcomes, policy_versions with dual-control fields, safety_evals)
- **Services:** ✓ Core infrastructure (ModalityDirector, ModalityValidationService, PolicyEngine, SafetyEval)
- **Contracts:** ✓ Shared types (ModalityKind, PolicyStatus incl. PROMOTION_REQUESTED/REJECTED, SafetyMetrics, PolicyChoiceRecord)
- **Golden Tests:** ✓ 16/16 passing (A01–A16); policy suites use cross-process lock
- **Learned Policy Runtime:** ⚠️ Stub (always returns baseline; deterministic test artifacts for A03/A04)
- **API Endpoints:** ✓ PolicyController (promotion/rollback); prod auth deferred
