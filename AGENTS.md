...
| **MVP 3.0** | [`/docs/mvp-3.0/`](./docs/mvp-3.0/) | [`/COGNA 3.0/`](./COGNA%203.0/) | **Active** — all new product work |
...
## MVP 3.0 — active source of truth

- **Spec:** `/docs/mvp-3.0/` for new work  
- **Build tracking:** `/COGNA 3.0/`  
- Prefer branch `feat/mvp-2.0-build` (see repo org doc)

### Mandatory tracking updates (MVP 3.0)

When you complete, skip, or discover work on Cogna MVP 3.0, **in the same turn** update:

1. [`COGNA 3.0/BUILD_PLAN.md`](./COGNA%203.0/BUILD_PLAN.md) — strikethrough finished tasks (`~~…~~`)  
2. [`COGNA 3.0/SKIPPED.md`](./COGNA%203.0/SKIPPED.md) — log intentional deferrals  
3. [`COGNA 3.0/BUILD_CARE.md`](./COGNA%203.0/BUILD_CARE.md) — add/check guardrails  
4. [`COGNA 3.0/README.md`](./COGNA%203.0/README.md) — keep status snapshot current  
...
- Active-era extensions only as specified in `/docs/mvp-3.0/` today — never invent flat UI action aliases. Future eras may add intents/parameters only after that era is Canonical and Active.
