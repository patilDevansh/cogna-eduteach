...
**Phase 0/2/3 progress, 18 September 2026 (later same day).** Additional §10 Phase 0, Phase 2, and part of Phase 3 work landed and passes 127/127 Lotus golden tests, including new tests run against a real local Postgres — not yet against staging/production or under a supervised pilot. Phase 0: `LotusEvidenceRecord`/`LotusSessionRecord` gained dedicated columns (queue/analysis timestamps, installed slot, shown ack, prompt/model/policy/skill-map versions, a DB-level unique `(session, question, submission)` duplicate-submit guard); a durable outbox (the existing generic `Job` table) now records every deferred-analysis and reserve-replenish kick-off, with a worker-authenticated drain endpoint (`POST /lotus/jobs/run-outbox`) that is a safe no-op when the in-process work already finished; an event-order reconciliation utility (`reconcileSession`, `GET /lotus/sessions/:id/reconcile`) cross-checks the stored snapshot against its own event log and flags real disagreement; an observer-only `?source=db` read bypasses the in-process cache; and pseudonymous learner IDs (deterministic HMAC), a `retentionUntil` field, and a real deletion/export pair (`DELETE`/`GET /lotus/students/:studentId`) are implemented — retention-window *enforcement* (an automated sweep) is not, since no scheduler infra exists yet. The AI Studio per-turn status strip (code-verified / AI-review queued-running-complete-failed / evidence source) is implemented in the observer view only. Phase 2: the `BROADEN` action is now actually produced (a `WIDEN` plan action reusing existing multi-shape catalogue items, never authoring new content) with its own KEEP/defer fallback; two real bugs found while adding it are fixed and covered by regression tests — the plan previously repurposed the *furthest* eligible slot instead of the nearest (the literal §8 Q24/Q25 incident), and a same-replan race could let a code-instant suspicion and a later AI request both install a check for the same skill. Phase 3: adaptive-decision items now carry a structured, observer-only `adaptationTag` (`Targeted check`, `Easier prerequisite`, `Broadened evidence`, `Coverage replacement` — no `Challenge/extension` action exists yet, so that tag is not offered) shown only on the specific installed item, never on an unchanged coverage question or the student-facing header (kept off the student view deliberately, since it would otherwise reveal which skill Lotus currently suspects); factorisation questions were confirmed already architecturally unable to serve `HARDCODED_SYSTEM` provenance (the browser-facing session view nulls out any non-AI-origin item). Not done: the full §11 AI Studio dashboard redesign (Session Overview / Turn Decision Card / Audit Drawer), the proposed→queued→validated→installed→shown status machine (§10 Phase 5), cost tracking, an automated retention sweep, and the Playwright suites in §9/§10 Phase 6 — those remain as scoped there.
