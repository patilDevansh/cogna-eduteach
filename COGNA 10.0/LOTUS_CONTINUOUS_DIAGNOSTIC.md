...
**Delivery checkpoint, 19 September 2026.** Phase 0 implementation now persists the session snapshot, accepted answer event, and idempotent deferred-work outbox entry in the same database transaction; recovery/reconciliation treats every accepted response — including a pending review or explicit support signal — as required evidence. Phase 1's evidence-calibration gate passed previously. Phase 2 now records an observer-visible, server-validated decision for every answer (`KEEP`, `TARGETED_PROBE`, `EASIER_PREREQUISITE`, `BROADEN`, `REMOVE_OR_DEFER`, or `STOP`), its evidence, alternatives, earliest requested slot, and its real state: applied, queued while an AI-written item is generated, or not applied with a reason. The Decision panel renders this below each final conclusion. The controlled Playwright wiring suite now checks that panel through the real student UI and API. **Phase 0 and Phase 2 test gates have not yet been rerun after this checkpoint and are not complete.** Phase 3 provenance/readiness work is present in the working branch but is out of scope for declaring the earlier phases done. Retention-window automation, full proposed→queued→validated→installed→shown history, latency controls, cost tracking, the full AI Studio redesign, live-model persona evaluation, and supervised pilot remain later work.
...
**Phase 3 question-bank implementation checkpoint, 20 September 2026.** A Factorisation item enters the durable AI question bank only when an active learner has submitted an answer to an AI-authored, code-validated item. The bank stores the item and non-identifying source metadata, never a learner response or analysis. A later session may reuse it only when it matches the planned skill, is not from the same session, does not duplicate that learner's test, and passes the current deterministic item checks again. The reuse is explicitly labelled in the student header, unseen plan, and AI Studio. Q1 remains newly generated for its session. Migration, bank-reuse, restart, and Playwright persona gates remain pending approval; this is not a completed Phase 3 gate.
